...
TEST(SelectStatement_Complex) {
    ...
    auto join_cond = std::make_unique<BinaryExpr>(
        std::make_unique<ColumnExpr>("users", "id"),
        TokenType::Eq,
        std::make_unique<ColumnExpr>("orders", "user_id")
    );
    stmt->add_join(SelectStatement::JoinType::Inner, std::make_unique<ColumnExpr>("orders"), std::move(join_cond));
    
    // LEFT JOIN metadata (no condition for test simplicity, though invalid SQL usually)
    stmt->add_join(SelectStatement::JoinType::Left, std::make_unique<ColumnExpr>("metadata"), nullptr);

    // WHERE age > 18
    stmt->set_where(std::make_unique<BinaryExpr>(
        std::make_unique<ColumnExpr>("age"),
        TokenType::Gt,
    ...
    stmt->set_having(std::make_unique<BinaryExpr>(
        std::move(count_func),
        TokenType::Gt,
        std::make_unique<ConstantExpr>(Value::make_int64(5))
    ));
    
    // ORDER BY name DESC (using simplified check since we don't have DESC enum exposed in expression easily here)
    ...
    // "SELECT DISTINCT id, name FROM users JOIN orders ON users.id = orders.user_id LEFT JOIN metadata WHERE age > 18 GROUP BY age HAVING COUNT(*) > 5 ORDER BY name LIMIT 10 OFFSET 5"
    
    EXPECT_STREQ(sql, "SELECT DISTINCT id, name FROM users JOIN orders ON users.id = orders.user_id LEFT JOIN metadata WHERE age > 18 GROUP BY age HAVING COUNT(*) > 5 ORDER BY name LIMIT 10 OFFSET 5");
}
...
TEST(UpdateStatement_Basic) {
    ...
    stmt->add_set(std::make_unique<ColumnExpr>("price"), std::make_unique<ConstantExpr>(Value::make_int64(100)));
    stmt->add_set(std::make_unique<ColumnExpr>("stock"), std::make_unique<ConstantExpr>(Value::make_int64(50)));
    
    stmt->set_where(std::make_unique<BinaryExpr>(
        std::make_unique<ColumnExpr>("id"),
        TokenType::Eq,
...
TEST(CreateTableStatement_Complex) {
    ...
    EXPECT_STREQ(stmt->to_string(), "CREATE TABLE complex_table (id INT PRIMARY KEY, name TEXT NOT NULL UNIQUE)");
}
