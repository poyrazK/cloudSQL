...
TEST(StatementTests, SelectStatementComplex) {
...
TEST(StatementTests, InsertStatementMultiRow) {
...
TEST(StatementTests, UpdateStatementBasic) {
    ...
    const std::string sql = stmt->to_string();
    EXPECT_NE(sql.find("price = 100"), std::string::npos);
    EXPECT_NE(sql.find("stock = 50"), std::string::npos);
}

TEST(StatementTests, DeleteStatementBasic) {
    ...
    EXPECT_STREQ(stmt->to_string().c_str(), "DELETE FROM users WHERE id < 0");
}

TEST(StatementTests, CreateTableStatementComplex) {
