...
class VectorizedSeqScanTests : public ::testing::Test {
 protected:
  void SetUp() override {
    storage_ = std::make_unique<StorageManager>("./test_scan_ops");
  }
...
class VectorizedProjectTests : public ::testing::Test {
 protected:
  void SetUp() override {
    storage_ = std::make_unique<StorageManager>("./test_project_ops");
  }
  void TearDown() override { storage_.reset(); }
...
TEST_F(VectorizedProjectTests, MultipleExpressions) {
  ...
  for (int64_t i = 0; i < 5; ++i) {
    batch->append_tuple(Tuple({common::Value::make_int64(i), common::Value::make_int64(i * 2)}));
  }
  ASSERT_TRUE(table.append_batch(*batch));
  ...
  std::vector<std::unique_ptr<Expression>> exprs;
  // a + b
  exprs.push_back(std::make_unique<BinaryExpr>(std::make_unique<ColumnExpr>("a"),
                                                TokenType::Plus, std::make_unique<ColumnExpr>("b")));
  // a * b
  exprs.push_back(std::make_unique<BinaryExpr>(std::make_unique<ColumnExpr>("a"),
                                                TokenType::Star, std::make_unique<ColumnExpr>("b")));
...
class VectorizedAggregateTests : public ::testing::Test {
 protected:
  void SetUp() override {
    storage_ = std::make_unique<StorageManager>("./test_agg_ops");
  }
  void TearDown() override { storage_.reset(); }
...
TEST_F(VectorizedAggregateTests, SumWithFloat64) {
  ...
  std::vector<VectorizedAggregateInfo> aggs = {
      {AggregateType::Count, -1}, {AggregateType::Sum, 0}};
...
class VectorizedFilterTests : public ::testing::Test {
 protected:
  void SetUp() override {
    storage_ = std::make_unique<StorageManager>("./test_filter_ops");
  }
  void TearDown() override { storage_.reset(); }
...
TEST_F(VectorizedFilterTests, FilterAllMatch) {
  ...
  auto cond = std::make_unique<BinaryExpr>(
      std::make_unique<ColumnExpr>("val"), TokenType::Ge,
