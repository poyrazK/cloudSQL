...
std::unique_ptr<AggregateOperator> make_agg_op(
    std::unique_ptr<Operator> child, std::vector<std::unique_ptr<Expression>> group_by,
...
TEST_F(OperatorTests, FilterBasic) {
  ...
  auto scan = make_buffer_scan("test_table", data, schema);
  // Filter: id >= 2
  auto filter = make_filter(std::move(scan),
                            binary_expr(col_expr("id"), TokenType::Ge,
                                        const_expr(common::Value::make_int64(2))));
...
TEST_F(OperatorTests, FilterAllFiltered) {
  ...
  auto filter = make_filter(std::move(scan),
                            binary_expr(col_expr("id"), TokenType::Gt,
                                        const_expr(common::Value::make_int64(100))));
...
TEST_F(OperatorTests, ProjectBasic) {
  ...
  data.push_back(
      make_tuple({common::Value::make_int64(2), common::Value::make_text("bob")}));
