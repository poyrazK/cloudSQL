...
std::unique_ptr<Statement> Parser::parse_select() {
    ...
    while (true) {
        ...
        if (!expr) {
            std::cerr << "Parser Error: Invalid column expression" << "\n";
            return nullptr;
        }
        stmt->add_column(std::move(expr));

        if (peek_token().type() == TokenType::From) {
            break;
        }
    }
    ...
    if (consume(TokenType::From)) {
        ...
        if (!from_expr) {
            std::cerr << "Parser Error: Invalid FROM expression" << "\n";
            return nullptr;
        }
        ...
        /* Optional JOINs */
        while (true) {
            SelectStatement::JoinType join_type = SelectStatement::JoinType::Inner;
    ...
    if (peek_token().type() == TokenType::Group) {
        static_cast<void>(consume(TokenType::Group));
        if (consume(TokenType::By)) {
            bool g_first = true;
            while (true) {
                if (!g_first && !consume(TokenType::Comma)) {
                    break;
                }
                g_first = false;
    ...
    if (peek_token().type() == TokenType::Having) {
        static_cast<void>(consume(TokenType::Having));
    ...
    if (peek_token().type() == TokenType::Order) {
        static_cast<void>(consume(TokenType::Order));
        if (consume(TokenType::By)) {
            bool o_first = true;
            while (true) {
                if (!o_first && !consume(TokenType::Comma)) {
                    break;
                }
                o_first = false;
...
std::unique_ptr<Statement> Parser::parse_create_table() {
    ...
    while (true) {
        ...
        const Token col_type = next_token();
        std::string type_str = col_type.lexeme();

        if (col_type.type() == TokenType::TypeVarchar) {
            if (consume(TokenType::LParen)) {
                const Token len = next_token();
                if (len.type() != TokenType::Number) {
                    return nullptr;
                }
                static_cast<void>(consume(TokenType::RParen));
                type_str += "(" + len.lexeme() + ")";
            }
        }
        ...
        while (true) {
            ...
            if (t.type() == TokenType::Primary) {
                static_cast<void>(consume(TokenType::Primary));
                ...
                static_cast<void>(consume(TokenType::Unique));
...
std::unique_ptr<Statement> Parser::parse_insert() {
    ...
    const Token table_tok = next_token();
    ...
    if (consume(TokenType::LParen)) {
        ...
        while (true) {
            ...
            const Token col_tok = next_token();
...
std::unique_ptr<Expression> Parser::parse_and() { // NOLINT(misc-no-recursion)
    ...
    while (peek_token().type() == TokenType::And) {
        static_cast<void>(consume(TokenType::And));
...
std::unique_ptr<Expression> Parser::parse_compare() { // NOLINT(misc-no-recursion)
    ...
    if (consume(TokenType::Is)) {
        const bool not_flag = consume(TokenType::Not);
        if (!consume(TokenType::Null)) {
            return nullptr;
        }
...
std::unique_ptr<Expression> Parser::parse_primary() { // NOLINT(misc-no-recursion)
    ...
    if (tok.type() == TokenType::Identifier || tok.is_keyword()) {
        ...
        if (peek_token().type() == TokenType::LParen) {
            static_cast<void>(consume(TokenType::LParen));
            ...
            if (peek_token().type() == TokenType::Distinct) {
                static_cast<void>(consume(TokenType::Distinct));
        ...
        if (peek_token().type() == TokenType::Dot) {
            static_cast<void>(consume(TokenType::Dot));
            const Token col_id = next_token();
...
std::unique_ptr<Statement> Parser::parse_drop() {
    if (!consume(TokenType::Drop)) {
        return nullptr;
    }

    bool if_exists = false;
    if (peek_token().type() == TokenType::Table) {
        static_cast<void>(consume(TokenType::Table));
